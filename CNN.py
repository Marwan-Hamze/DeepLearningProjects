...
parser.add_argument("operation", type= str, choices=["train", "load"], help= "train and save the model, or load an existing one")

args, remaining_argv = parser.parse_known_args()

# Assigning Variables used in both training and loading models
...
if args.operation == "train":
    ...
    MODEL_PATH = Path("Computer_Vision_Models")
    MODEL_PATH.mkdir(parents=True, # create parent directories if needed
                    exist_ok=True # if models directory already exists, don't error
        )

    # Create model save path
    MODEL_NAME = "Mnest_CNN.pth"
    MODEL_SAVE_PATH = MODEL_PATH / MODEL_NAME
    # Save the model state dict. For saving the model vs its state_dict, see Notes.txt
    ...
    loaded_model = CNN(1, hidden_layers, 10) 

    # Load in the saved state_dict()
    loaded_model.load_state_dict(torch.load(f=LOAD_PATH))
